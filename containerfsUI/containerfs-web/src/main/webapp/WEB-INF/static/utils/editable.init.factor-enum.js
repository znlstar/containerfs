...
(function ($) {
    ...
    var EditableTable = {
        ...
        rowEdit: function ($row) {
            var _self = this, data;
            var enum_names = ["enumId", "enumName", "enumCode", "enumKey", "dwKey", "enumValue", "action"];    //字段段名 by wangjufnu
            ...
            $row.children('td').each(function (i) {
                ...
                if ($this.hasClass('actions')) {
                    ...
                    if (i == 0) {
                        // id 不可修改
                        $this.html('<input type="text" disabled="disabled" name="' + enum_names[i] + '" class="form-control input-block" value="' + data[i] + '" />');
                    } else {
                        $this.html('<input type="text" name="' + enum_names[i] + '" class="form-control input-block" value="' + data[i] + '" />');
                    }
                }
            });
        },
        ...
        rowSave: function ($row) {
            ...
            $.ajax({
                ...
                success: function (data) {
                    ...
                    if (r.returnCode == "1") {
                        // 提交成功 刷新左树 刷新列表页
                        getFactorEnumLeftTree();
                        factorEnumListUrl(r.enumCode);
