(function($){
	$.fn.onLine = function(options){
		...
		//初始化赋值 列表内容
		box.find(".showleft").children("tbody").children("tr").each(function(index, element) {
			$(this).attr({group:"gpl",left:$(this).position().left+$(this).outerWidth(),top:$(this).position().top+$(this).outerHeight()/2,sel:"0",check:"0"});
		});
		box.find(".showright").children("tbody").children("tr").each(function(index, element) {
		...
		part2.attr('first',0);
		//canvas 赋值
		var canvas =box.find(".canvass")[0];  //获取canvas  实际连线标签
		canvas.width=$(".card-box").width();//canvas宽度等于div容器宽度
		canvas.height=$(".card-box").height();
		var backcanvas =box.find(".backcanvass")[0];  //获取canvas 模拟连线标签
		backcanvas.width=$(".card-box").width();
		backcanvas.height=$(".card-box").height();
		//连线数据
		...
		//addstyle showitem
		box.find("tr").on("mousedown", function(event){
			countL = 0;
			countR = 0;
		...
		$(document).mousemove(function(event){		//移动
			...
			if(groupstate){
				...
				var targettrue=null;
				if($target.is("td") && $target.closest(".show").parent().attr("class")==box.attr("class") ){
					targettrue=$target;
				}else if($target.closest(".showitem").is("tr") && $target.closest(".show").parent().attr("class")==box.attr("class")){
