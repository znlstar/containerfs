$(function () {
    ...
    $.Validator.match = function (para) {
        //定义默认的验证规则
        var defaultVal = {
            SDATE: "^(\\d{4})-(\\d{1,2})-(\\d{1,2})$",  //短型日期格式
            FLOAT: "^\\d+(\\.\\d+)?$",                  //非负浮点数（正浮点数 + 0）
            FLOATS: "^(-?\\d+)(\\.\\d+)?$",             //浮点数
            NUMBER: "^[0-9]*$",                         //正整数（包括0）
            NUMERIC: "^(-)?[0-9]*$",                    //正负整数（包括0）
            ONLYCHAR: "^[A-Za-z]+$",                    //只能输入26个字母
            CHARANDNUMBER: "^[A-Za-z0-9_]+$",           //只能输入字母数字和下划线
            TEL: "^0(10|2[0-5789]|\\d{3})-\\d{7,8}$",
            IP: "^((\\d|[1-9]\\d|1\\d\\d|2[0-4]\\d|25[0-5]|[*])\\.){3}(\\d|[1-9]\\d|1\\d\\d|2[0-4]\\d|25[0-5]|[*])$",
            MOBILE: "^1(3[0-9]|5[0-35-9]|7[0-9]|8[0235-9])\\d{8}$",
...
function validateBefore() {
    ...
    if (valType == 'OTHER') {
        ...
        //如果类型不是自定义，则匹配默认的验证规则进行验证
        if ($(this).attr('valType') == 'required') {
            //不能为空的判断
            if ($(this).val() == '') {
                flag = false;
            }
